...
class BrTimingSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request(
            url="https://coderesearch.com/sts/services/10020", callback=self.get_years
        )

    def get_years(self, response):
        years = (
            response.xpath('//select[@id="year"]').css("option::attr(value)").getall()
        )
        ...
        for year in years:
            if year != "":  # and year == "2018":
                ...
                yield scrapy.Request(
                    url=url, callback=self.parse_year, meta={"year": int(year)}
                )
    ...
    def parse_event(self, response):
        contests = (
            response.xpath('//select[@id="contest"]')
    ...
    def parse_contest(self, response):
        ...
        elif len(distances_in_string) >= 2 and all(
            x == distances_in_string[0] for x in distances_in_string
        ):
            distance = float(distances_in_string[0].replace(",", "."))
        else:
            distance = None
        ...
        for competitor in response.xpath('//table[@class="results"]/tbody/tr')[:]:
            if competitor.css("tr::attr(class)").get() in ["lead", "subsidiary"]:
                ...
                relay = False
            place_total = (
                competitor.css("td[class=col-place-total]")
                .css("a")
                .css("div::text")
                .get()
            )
            place_sex = (
                competitor.css("td[class=col-place-sex]")
                .css("a")
                .css("div::text")
                .get()
            )
            place_ageclass = (
                competitor.css("td[class=col-place-ageclass]")
                .css("a")
                .css("div::text")
                .get()
            )
            bib_number = (
                competitor.css("td[class=col-number]").css("a").css("div::text").get()
            )
            name = (
                competitor.css("td[class=col-competitor]")
                .css("a")
                .css("div::text")
                .get()
            )
            ...
            birth = (
                competitor.css("td[class=col-birth]").css("a").css("div::text").get()
            )
            nationality = (
                competitor.css("td[class=col-nationality]").css("a::text").get()
            )
            ageclass = (
                competitor.css("td[class=col-ageclass]").css("a").css("div::text").get()
            )
            time = competitor.css("td[class*=col-time]").css("a::text").get()
            time_net = competitor.css("td[class*=col-net-time]").css("a::text").get()
            laps = (
                competitor.css("td[class*=col-lap-count-interims]").css("a::text").get()
            )

            result_url = (
                competitor.css("td[class=col-competitor]").css("a::attr(href)").get()
            )
